<%@ Page Title="" Language="C#" MasterPageFile="~/Header_Footer_Admin.Master" AutoEventWireup="true" CodeBehind="LoaiSP.aspx.cs" Inherits="QLBH_TeemoSneaker.Page_Admin.LoaiSP" %>
...
<i class="bi bi-file-earmark-spreadsheet-fill"></i> Xuất Excel
        ...
        <div class="item-table">
            <asp:UpdatePanel ID="UpdatePanel1" runat="server">
                <ContentTemplate>
                    <asp:GridView ID="dsSanPham" runat="server" DataKeyNames="MaLoaiGiay" CssClass="GridViewStyle"
                        ...
                        <Columns>
                            <%--mã loại giày--%>
                            <asp:BoundField DataField="MaLoaiGiay" HeaderText="Mã loại" ReadOnly="True">
                                <ItemStyle Width="100px" />
                            </asp:BoundField>

                            <%--tên loại giày--%>
                            <asp:TemplateField HeaderText="Loại giày">
                                <ItemStyle Width="200px" />
                                <ItemTemplate>
                                    <asp:Label ID="lblTenLoaiGiay" runat="server"
                                        Text='<%# Eval("TenLoaiGiay") %>' />
                                </ItemTemplate>
                                <EditItemTemplate>
                                    <asp:TextBox ID="txtTenLoaiGiay" runat="server"
                                        Text='<%# Bind("TenLoaiGiay") %>' CssClass="form-control" 
                                         TextMode="MultiLine" Rows="2"
                                        Style="text-align: justify; width:180px;"/>
                                </EditItemTemplate>
                            </asp:TemplateField>

                            <%--đặc điểm--%>
                            <asp:TemplateField HeaderText="Đặc điểm">
                                <ItemTemplate>
                                    <asp:Label ID="lblDacDiem" runat="server"
                                        CssClass="wrap-text"
                                        Text='<%# Eval("DacDiem") %>'
                                        ToolTip='<%# Eval("DacDiem") %>'
                                        Style="text-align: justify; margin-right: 20px;" />
                                </ItemTemplate>
                                <EditItemTemplate>
                                    <asp:TextBox ID="txtDacDiem" runat="server"
                                        Text='<%# Bind("DacDiem") %>'
            ...
            <div id="popupForm" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 9999; display: flex; justify-content: center; align-items: center;">
                <div class="table-content" style="background: #fff; padding: 30px; border-radius: 10px; width: 500px; position: relative;">
                    ...
                    <h4 style="margin-bottom: 30px;">THÊM LOẠI SẢN PHẨM</h4>

                    <!-- TenLoaiGiay -->
                    <div class="box-content">
                        <div class="title-content">
                            <div class="input-group">
                                <asp:TextBox ID="txtTenLoaiGiayMoi" runat="server" CssClass="input" />
                                <label class="user-label" for="txtTenLoaiGiayMoi">Tên loại giày</label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="box-content">
                        <div class="title-content">
                            <div class="input-group">
                                <asp:TextBox ID="txtDacDiemMoi" runat="server" CssClass="input" TextMode="MultiLine" Rows="3" />
                                <label class="user-label" for="txtDacDiemMoi">Đặc điểm</label>
                            </div>
                        </div>
                    </div>
    ...
    <asp:SqlDataSource ID="SqlDataSourceLoaiGiay" runat="server"
            ...
            SelectCommand="SELECT * FROM tblLOAIGIAY"
            InsertCommand="INSERT INTO tblLOAIGIAY (MaLoaiGiay, TenLoaiGiay, DacDiem) VALUES (@MaLoaiGiay, @TenLoaiGiay, @DacDiem)"
            UpdateCommand="UPDATE tblLOAIGIAY SET TenLoaiGiay = @TenLoaiGiay, DacDiem = @DacDiem WHERE MaLoaiGiay = @MaLoaiGiay"
            DeleteCommand="DELETE FROM tblLOAIGIAY WHERE MaLoaiGiay = @MaLoaiGiay">
            
            <%--tham số thêm--%>
            <InsertParameters>
                <asp:Parameter Name="MaLoaiGiay" Type="String" />
                <asp:Parameter Name="TenLoaiGiay" Type="String" />
    ...
    <script type="text/javascript">
        ...
        function resetTextBoxes() {
            document.getElementById("<%= txtTenLoaiGiayMoi.ClientID %>").value = "";  // Reset Tên loại giày
                document.getElementById("<%= txtDacDiemMoi.ClientID %>").value = "";  // Reset Đặc điểm
        }
